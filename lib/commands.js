...
const notifyCommand = {
	input: function(client, target, command, args) {
		...
		switch (subcommand) {
			...
			case "version":
				return handleVersion(realClient, network);
...
/**
 * Show plugin version
 * Usage: /notify version
 */
function handleVersion(client, network) {
	const packageJson = require("../package.json");
	const version = packageJson.version || "unknown";
	const name = packageJson.thelounge?.name || "External Notify";

	const messages = [
		F.BREAK,
		F.LABEL('Plugin', `${C.BOLD}${name}${C.RESET}`),
		F.LABEL('Version', `${C.CYAN}v${version}${C.RESET}`),
		F.LABEL('Repository', packageJson.repository?.url || 'N/A')
	];
	...
	if (packageJson.repository && packageJson.repository.url) {
		const { checkForUpdate } = require("./version-check");
		checkForUpdate(packageJson.version, packageJson.repository.url)
			.then(updateInfo => {
				if (updateInfo.updateAvailable) {
					...
					sendMessage(client, network, [
						F.BREAK,
						F.SUCCESS('✓ You are running the latest version')
					]);
				}
			})
			.catch(err => {
				// Show error only in version command since user explicitly asked
				sendMessage(client, network, [
					F.BREAK,
					F.INDENT(1, `${C.DISABLED}(Unable to check for updates: ${err.message})${C.RESET}`)
				]);
			});
	}
}
...
function handleHelp(client, network) {
	...
	messages.push(
		...
		F.INDENT(1, F.CMD('version', 'Show plugin version')),
